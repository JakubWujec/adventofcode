...
function z1(input){
    ...
    let scaffoldImage = new ScaffoldImage(shipComputer.outputs);
    scaffoldImage.display();

    let intersections = scaffoldImage.getIntersectionsPositions();
    let alignmentParameter = intersections.map(list => list[0] * list[1]).reduce((a,b) => a+b, 0);
...
function z2(input){
    ...
    let scaffoldImage = new ScaffoldImage(shipComputer.outputs);
    scaffoldImage.display();
    let startPosition = scaffoldImage.getStartPosition();
    let steps = scaffoldImage.getSteps(startPosition);
    let solution = scaffoldImage.divideSteps(steps);
    let inputs = (solution.map(input => input.map(x => x.toString()).join(',')));
    for(let func of inputs){
        for(let char of func){
            shipComputer.addInputs([char.charCodeAt(0)])
        }
        shipComputer.addInputs(['\n'.charCodeAt(0)])
    }
    shipComputer.addInputs(['\n'.charCodeAt(0)]);
    shipComputer.addInputs(['\n'.charCodeAt(0)]);
    shipComputer.run();
    return shipComputer.outputs[shipComputer.outputs.length - 1]
